...
# This dictionary defines the properties for each type of note.
NOTE_CATEGORIES = {
...
def migrate_database_to_phase3(db_path):
    ...
    # Add columns to the embedding_status table
    embedding_columns = [
...
def save_advanced_note(note_type, title, content, links="", tags="", audio_file=None):
    ...
    if NOTE_CATEGORIES[note_type]["needs_embedding"]:
        cursor.execute("INSERT OR REPLACE INTO embedding_status (file_path, needs_embedding, file_hash, processing_status) VALUES (?, ?, ?, ?)",
...
def get_advanced_notes_stats():
    ...
    try:
        ...
        needs_embedding_types = [k for k, v in NOTE_CATEGORIES.items() if v["needs_embedding"]]
        needs_embedding = conn.execute(f"SELECT COUNT(*) FROM notes WHERE has_embedding = FALSE AND note_type IN ({','.join(['?']*len(needs_embedding_types))})", needs_embedding_types).fetchone()[0]
    ...
    try:
        notes = conn.execute("""
            SELECT id, note_type, title, content_path, COALESCE(links, '') as links, COALESCE(tags, '') as tags,
                   ...
                   COALESCE(embedding_model, '') as embedding_model, COALESCE(file_size, 0) as file_size,
        ...
        return [{"id": n[0], "type": n[1], "title": n[2], "content_path": n[3], "links": n[4], "tags": n[5],
                 "timestamp": n[6], "last_modified": n[7], "has_embedding": n[8], "embedding_model": n[9],
                 "file_size": n[10], "search_priority": n[11]} for n in notes]
    finally:
        conn.close()
...
def delete_note(note_id):
    """Delete a note from the database and file system."""
    db_path = Path("notes") / "metadata" / "notes_database.db"
    conn = sqlite3.connect(str(db_path))
    try:
