...
if not st.session_state.drive_synced:
    ...
    if drive:
        ...
        st.success("✅ Synced with Google Drive!")
        # Rerun to ensure the rest of the app loads with the synced data
        st.rerun()
    else:
        st.error("❌ Google Drive sync failed. App cannot continue.")
        st.stop() # Stop the app if sync fails
# --- END GOOGLE DRIVE SYNC ---

# Create vertical tabs
tab1, tab2, tab3, tab4, tab5 = st.tabs(["Search", "Notes", "Configuration", "Data Management", "About"])
...
with tab2:
    ...
    if stats.get("pending_jobs", 0) > 0:
        if st.button("🚀 Process Embedding Queue (with Progress)"):
    ...
    with st.expander("✨ Create Advanced Note", expanded=False):
    ...
    with st.expander("🗂️ Manage All Notes", expanded=False):
    ...
    if stats["total_notes"] > 0:
        ...
        for note in recent:
            ...
            with st.expander(f"{emoji} {note['title'][:20]}..."):
...
with tab4:
    ...
    with col3:
        if st.button("🗑️ Clear Collections"):
            if st.button("⚠️ Confirm Clear Collections"):
                try:
